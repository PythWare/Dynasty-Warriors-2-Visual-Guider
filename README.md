...
An optional toggle for morale bars has been added. Allow me to explain. Every squad now has a morale bar, blue for side 1 and red for side 2. It gets filled based on the morale of the force leader. If the force leader has the morale changed then their morale bar as well as any subofficers, troops, etc that serve either the force leader or one of the underlings has their morale bar updated dynamically to match the force leader. The morale bar moves with the squads, that way nothing is lost visually. The video shows the morale bars dynamically updating based on current values you assign/allow.

You can mod morale data.

There is now a global morale bar (as you see in the video in the Squad Editor section), it dynamically updates based on the total morale of side 1 and side 2. Every stage now has a global morale bar in Visual Guider.
